...
namespace _1_InputValidation
{
    public partial class Form1 : Form
    {
        ...
        readonly string phoneMsg = "The phone number is not a valid phone number";
        ...
        {
            if (!Regex.IsMatch(textBox_Name.Text, nameRegex)) MessageBox.Show(nameMsg);

            if (!Regex.IsMatch(textBox_Phone.Text, phoneRegex)) MessageBox.Show(phoneMsg);

            if (!Regex.IsMatch(textBox_Email.Text, emailRegex)) MessageBox.Show(emailMsg);
        }
